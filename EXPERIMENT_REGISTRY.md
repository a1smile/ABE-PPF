...
| 0.02 | 192 | 17 | 53.13% | 1.999 | 0.545 | same fails, 2× slower |
| 0.03 | 192 | 17 | 53.13% | 1.980 | 0.515 | same fails, 2× slower |
...
3. **max=192 ×2 slower with no benefit.** Same 17/32, same failure pattern, 0.545s BRPMR.
4. **max=256 degrades** (16/32). Candidate explosion (256 per round) pollutes mode pool. New failure: obj_000010_f0.
5. **no-CF (max=0) reaches 18/32** but at 0.421s BRPMR (2× baseline). The extra success comes entirely from unlimited per-round candidates, not from ratio=0.00.
