...
from src.datasets.lmo import LMOSample, load_lmo_samples
from src.ppf.pipeline.ambiguity_budget_ppf import AmbiguityBudgetPPF, RuntimeSample
from src.utils.logger import setup_logger


def _to_runtime_sample(sample: LMOSample) -> RuntimeSample:
...
def main() -> None:
    ...
    ap.add_argument("--config", default=str(ROOT / "configs" / "lmo_small.yaml"))
    args = ap.parse_args()
    ...
    logger.info("Running LMO evaluation: %s", config_path.name)

    samples = load_lmo_samples(
        manifest_path=str(cfg["manifest_path"]),
        max_objects=int(cfg.get("max_objects", 2)),
        max_frames_per_object=int(cfg.get("max_frames_per_object", 2)),
        t_scale=float(cfg.get("t_scale", 1.0)),
    )
    logger.info("Loaded %d LMO samples", len(samples))

    pipeline = AmbiguityBudgetPPF(cfg, logger=logger)
    cases = []
    failures = []
    for sample in samples:
        logger.info("Case obj=%s frame=%s variant=%s", sample.object_name, sample.frame_id, sample.scene_variant)
    ...
    logger.info("Saved LMO results to %s", result_path)
