...
from ppf.going_further_ppf import (
    _apply_close_pair_retention_sampling,
    _method_specific_target_spacing,
    build_going_further_model,
    run_going_further_registration,
)
...
def _cache_signature_payload(model_path: str, cfg: dict) -> dict:
    ...
    return {
        ...
        "going_further_ppf": dict(cfg.get("going_further_ppf", {}) or {}),
    }


def derive_going_further_cache_path(model_path: str, cfg: dict, cache_dir: str) -> str:
...
def build_or_load_going_further_cache(model_path: str, cache_path: str, cfg: dict, logger=None, rebuild: bool = False) -> str:
    if (not rebuild) and os.path.exists(cache_path):
        try:
            cached_model = load_ppf_model_cache(cache_path, cfg=cfg, strict=False)
            if hasattr(cached_model, "going_further_meta"):
                if logger:
                    logger.info(f"[GoingFurtherCache] reuse {cache_path}")
                return cache_path
            if logger:
                logger.warning(f"[GoingFurtherCache] cache missing metadata, rebuilding: {cache_path}")
        except Exception as e:
            if logger:
                logger.warning(f"[GoingFurtherCache] invalid cache, rebuilding: {cache_path} ({e})")
    ...
    gf_cfg = dict(cfg.get("going_further_ppf", {}) or {})
    use_method_specific_sampling = bool(gf_cfg.get("use_method_specific_sampling", True))
    method_specific_sampling_apply_to = str(gf_cfg.get("method_specific_sampling_apply_to", "model")).lower()
    ...
    if logger:
        logger.info(f"[GoingFurtherCache] build model={model_path} downsample={ds_info}")
    ...
    ppf_model = build_going_further_model(
        ...
        distance_step,
        cfg,
        sampling_min_distance=float(ds_info.get("voxel_used", sampling_leaf)),
        logger=logger,
    )
    meta = make_cache_meta(_model_identifier(model_path), cfg)
    save_ppf_model_cache(cache_path, ppf_model, meta)
    if logger:
        logger.info(f"[GoingFurtherCache] saved {cache_path}")
    return cache_path
...
def process_one(task):
    ...
    try:
        ...
        T_pred, out_model, debug, stats = run_going_further_registration(
...
def main():
    ap = argparse.ArgumentParser(description="Stanford batch runner for the Going Further with PPF reproduction.")
    ap.add_argument("--config", type=str, default="configs/Stanford/going_further_ppf_stanford.yaml")
    ap.add_argument("--csv", type=str, required=True)
    ap.add_argument("--models_dir", type=str, default="")
    ap.add_argument("--cache_dir", type=str, default="data/stanford_bunny_ppf/model_cache_going_further")
    ap.add_argument("--rebuild_cache", action="store_true")
    ap.add_argument("--out_prefix", type=str, default="stanford_going_further_ppf")
    ...
    for idx, row in df.iterrows():
        ...
        cache_path = derive_going_further_cache_path(model_path, cfg, cache_dir)
    ...
    logger.info("Method: Going Further with PPF")
    ...
    for model_path, cache_path in tqdm(unique_models.items(), total=len(unique_models), desc="BuildCache", unit="model"):
        build_or_load_going_further_cache(model_path, cache_path, cfg, logger=logger, rebuild=bool(args.rebuild_cache))
